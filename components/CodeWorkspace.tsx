...
import { FileExplorerSidebar } from './workspace/FileExplorerSidebar';
...
const CodeWorkspace: React.FC<CodeWorkspaceProps> = ({ files, onClose, activeFileOverride }) => {
  ...
  useEffect(() => {
    if (activeFileOverride && files[activeFileOverride]) setActiveFile(activeFileOverride);
  ...
  const isWriting = currentFile?.content === '';
  ...
  return (
    <div className="flex h-full bg-slate-950 text-slate-300 rounded-[1.5rem] border border-slate-800 overflow-hidden shadow-2xl animate-in slide-in-from-right-10 duration-500">
      <FileExplorerSidebar files={files} activeFile={activeFile} onSelectFile={setActiveFile} onClose={onClose} />

      <div className="flex-1 flex flex-col min-w-0">
        <div className="h-12 bg-slate-900/80 border-b border-slate-800 flex items-center justify-between px-6">
          <div className="flex items-center gap-3">
            <div className={`w-2 h-2 rounded-full ${isWriting ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500'}`} />
